...
# Função para verificar se o filme está na Netflix Brasil
def esta_na_netflix(movie_id):
    ...
    for provider in flatrate:
        if 'netflix' in provider.get('provider_name', '').lower():
            return True
    return False

# Buscar filmes populares (TMDb popular + Netflix disponível no BR)
def get_filmes_netflix_populares():
...
# Pesquisar filmes na Netflix por título
def pesquisar_filmes_netflix(search_query):
...
# Buscar filme no megafilmeshdz.cyou para assistir
...
def index():
    ...
    if search:
        filmes = pesquisar_filmes_netflix(search)
    else:
        filmes = get_filmes_netflix_populares()
